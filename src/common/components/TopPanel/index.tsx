...
export const TopPanel: FC<Props> = ({ hiddenSearc }) => {
  ...
  const { api: { setSearchLine }, searchLine } = useNewsStore()
  ...
  return (
    <AppBar position="static">
      <Toolbar>
        <Typography style={{ cursor: 'pointer' }} onClick={() => navigete('/')} variant="h6" component="div" sx={{ flexGrow: 1 }}>
          News Site
        </Typography>
        {!hiddenSearc && (
          <Paper
          ...
          {user ? (
            <Box flexDirection="column">
              ...
              <Menu
                ...
                onClose={handleClose}
              >
                <MenuItem onClick={() => navigete(`/user/${user.id}`)}>
                  Профиль
                </MenuItem>
